...
from custom.memnet.data_utils import featurize_bot_act, featurize_user_act, featurize_query, len_bot_featurized_vec, \
    len_user_featurized_vec, utterance_len
...
class MemNetPolicy(Policy):
    ...
    """
    def __init__(self, history, query, label, batch_indexes, featurizer, model=None, embedding_size=10, hops=3,
        ...
        :param featurizer: Featurizer object. It's encode function will be called during inference to featurize user
        input
    ...
    def predict_action_probabilities(self, tracker, domain):
        if isinstance(tracker.events[-1], ActionExecuted):
            ...
            return p
        query, history = self.featurizer.encode(tracker, self.mem_size)
    ...
    def persist(self, path):
        ...
        with open(join(path, 'MemoryNetwork_metadata.pickle'), 'wb') as metadata_fh:
            pickle.dump((self.history, self.queries, self.labels, self.batch_indexes, self.model.hops,
                         self.model.embedding_size, self.current_epoch, self.model_name, self.featurizer), metadata_fh)
    ...
    def load(cls, path, featurizer, max_history):
        ...
        with open(join(path, 'MemoryNetwork_metadata.pickle'), 'rb') as metadata_fh:
            history, queries, labels, batch_indexes, hops, embedding_size, current_epoch, model_name, featurizer = \
        ...
        mem_net_policy = cls(history, queries, labels, batch_indexes, model=model,
                             embedding_size=embedding_size, hops=hops, featurizer=featurizer)
